...
void i2cWaitForComplete(u08 *status)
{
	// wait for i2c interface to complete operation
//	while( !(inb(TWCR) & BV(TWINT)) );
    // 슬래이브가 연결되어 있지 않다면 무한 루프가 발생하므로 통신이 지연되면 중단시킨다.
    *status = 0;
    while( !(inb(TWCR) & BV(TWINT)) ){
        *status = *status + 1;
        if(*status > I2C_DELAY_MAX){
//        	DBG1(0xE1, (uchar *)status, 1);
            return;
        }
    }
}
...
//	DBG1(0xE2, (uchar *)data, 2);
	...
	// leave with TWEA on for slave receiving
	i2cSendStop();
//	while( !(inb(TWCR) & BV(TWSTO)) );  // 여기에서 멈추는 경우가 있어 제거; 보내기만 하므로 작동에는 이상이 없다
